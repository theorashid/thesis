...
The problem is exacerbated for the young ages or causes with lower mortality, where the number of deaths might be zero, or for smaller geographical units, where the population might be zero.
As a demonstration, @fig-ch-4-sim shows the expected and simulated number of deaths for a young age group (1-4 years) and an old age group (80-84).
...
![Simulated and expected deaths from 2002 to 2019 for a young age group (1-4 years) and an old age group (80-84), assuming the national death rates in England and a population size of 1000.](../thesis-analysis/thesis_analysis/eda/figures/age_mx_time_sim.pdf){#fig-ch-4-sim}
...
$\nu_{st}$ and $\gamma_{at}$ allow space- and age group-specific non-linearity in the time trends.
...
I tried different probabilistic programming languages across `R`, `python` and `Julia`, in particular packages that implemented the more efficient No U-Turn Sampler (NUTS) [@rashidProbabilisticprogrammingpackages2022].
...
I have paid a lot of attention into open sourcing code for all analyses during my PhD.
